...
class Bag(NamedTuple):
    feats: typing.Optional[torch.Tensor]
    coords: typing.Optional[torch.Tensor]
    label: typing.Dict[str, torch.Tensor]
    meta: typing.Dict[str, torch.Tensor]


class BagLabelComputer(abc.ABC):
    @abc.abstractmethod
    def compute_bag_label(
        self, instance_labels: torch.Tensor, pos: torch.Tensor = None
...
class TargetNumbersBagLabelComputer(BagLabelComputer):
    def __init__(
        self,
        target_numbers: typing.Union[int, typing.Tuple[int]],
        min_instances_per_target: int = 1,
    ):
    ...
    def compute_bag_label(self, instance_labels, *args, **kwargs):
        ...
        bag_label = (count_per_target >= self.min_instances_per_target).all()
        if not bag_label:
...
class DistanceBagLabelComputer(BagLabelComputer):
    ...
    def _key_instances(self, instance_labels, pos):
        nodes = [(i, {"label": label, "loc": loc}) for i, (label, loc) in enumerate(zip(instance_labels, pos))]
        ...
        return np.array(sorted(set(itertools.chain.from_iterable(key_cliques))))
...
class DistanceBasedTargetNumbersBagLabelComputer(DistanceBagLabelComputer):
    def __init__(
        self,
        target_numbers: typing.Tuple[int],
        dist_predicate: typing.Callable[[int], bool],
    ):
        super().__init__(
            lambda a, b, dist: dist_predicate(dist) and a in target_numbers and b in target_numbers and a != b
        )
...
class Digits(data_utils.Dataset):
    ...
    def __init__(
        self,
        bag_label_computer: BagLabelComputer,
        num_digits: int = 10,
        min_bag_size: int = 2,
        mean_bag_size: int = 10,
        var_bag_size: int = 2,
        num_bags: int = 250,
        seed: int = 1,
        train: bool = True,
        ...
        for positive, bags in [(True, positive_bags), (False, negative_bags)]:
            for _ in range(self.num_bags // 2):
                bag_size = int(self.r.normal(self.mean_bag_size, self.var_bag_size))
                bag_size = max(bag_size, self.min_bag_size)
                while True:
                    instance_labels = torch.from_numpy(self.r.randint(0, self.num_digits, bag_size))
    ...
    def __getitem__(self, index) -> Bag:
        ...
        return Bag(
            ...
            coords=None,
            label={"bag": bag_label},
            meta={"instance_labels": instance_labels, "key_instances": key_instances},
        )
...
class OneHotMNISTBags(Digits):
    def __getitem__(self, index) -> Bag:
        ...
        feats = torch.nn.functional.one_hot(bag.meta["instance_labels"], self.num_digits).float()
        return bag._replace(feats=feats)
...
def load_mnist_instances(
    train: bool = True,
    r: np.random.RandomState = np.random,
    shuffle: bool = True,
    normalize: bool = True,
) -> typing.Dict[int, torch.Tensor]:
    transforms = [T.ToTensor()]
    if normalize:
        transforms.append(T.Normalize((0.1307,), (0.3081,)))
    ds = datasets.MNIST("../datasets", train=train, download=True, transform=T.Compose(transforms))
    loader = data_utils.DataLoader(ds, batch_size=len(ds), shuffle=False)
...
class DigitCollage(data_utils.Dataset):
    ...
    def __init__(
        ...
        for positive, bags in [(True, positive_bags), (False, negative_bags)]:
            for _ in range(self.num_bags // 2):
                ...
                bag_size = max(bag_size, self.min_bag_size)
                while True:
                    instance_labels, pos = self._propose_locations_and_labels(bag_size)
                    bag_label, _ = self.compute_bag_label(instance_labels, pos)
                    if bag_label == (1 if positive else 0):
    ...
    def _propose_locations_and_labels(self, bag_size, padding=28 // 2):
        ...
        while not self._valid_locations(
            pos := self.r.uniform(padding, self.collage_size - padding, size=(bag_size, 2)),
    ...
    def __getitem__(self, index) -> Bag:
        instance_labels, coords = self.bags[index]
        bag_label, key_instances = self.compute_bag_label(instance_labels, coords)
        return Bag(
            feats=None,
            coords=None,
            label={"bag": bag_label},
    ...
    def __init__(self, *args, **kwargs):
        ...
        for instance_labels, pos in self.bags:
            ...
            for label in instance_labels:
                label = label.item()
                bag_imgs.append(mnist_instances[label][instance_indices[label]])
                instance_indices[label] += 1
...
def make_collage(bag: Bag, collage_size: int = None) -> torch.tensor:
    if collage_size is None:
        collage_size = bag.coords.max().int() + 28

    collage_img = torch.zeros((collage_size, collage_size))
    for img, (x, y) in zip(bag.feats, bag.coords.int()):
...
if __name__ == "__main__":
    for train in [True, False]:
        loader = data_utils.DataLoader(
            MNISTBags(
                TargetNumbersBagLabelComputer(9, min_instances_per_target=2),
                mean_bag_size=10,
                var_bag_size=2,
                num_bags=100,
                seed=1,
        ...
        for i, bag in enumerate(loader):
            len_bag_list.append(bag.feats.shape[0])
            positive_bags += bag.label["bag"].float().item()
        print(f"{'Train' if train else 'Test'} dataset:")
        print(f"  number positive bags: {positive_bags:.0f}/{len(loader)}")
        print(
