...
def apply(x: T, /,
          ...
          module_func: typing.Callable[[
              torch.nn.Module], torch.nn.Module] = None,
          pyg_func: typing.Callable[[pyg_Data], pyg_Data] = None,
          throw: bool = True,
          **kwargs) -> T:
    if module_func is None:
        module_func = tensor_func
    recurse = functools.partial(
        apply, tensor_func=tensor_func, module_func=module_func, throw=False, **kwargs)
    if isinstance(x, torch.Tensor):
        return tensor_func(x, **kwargs)
    elif isinstance(x, torch.nn.Module):
        return module_func(x, **kwargs)
    elif pyg_Data is not None and isinstance(x, pyg_Data) and pyg_func is not None:
...
def device(x: T, dev: str = DEVICE, throw: bool = True) -> T:
    ...
    return apply(x,
                 ...
                 pyg_func=(lambda x, **_:
                           pyg_Data.to(x, dev)) if pyg_Data is not None else None,
                 throw=throw)
